 ...
 */
#include "PkgRaplLimitIVBWidget.h"
#include "pwtClientCommon/UILogger.h"

namespace PWT::CUI::INTEL {
    PkgRaplLimitIVBWidget::PkgRaplLimitIVBWidget() {
        ...
        QObject::connect(pl1Enable, &ConsoleCheckbox::checkStateChanged, this, &PkgRaplLimitIVBWidget::onPl1EnableChanged);
        QObject::connect(pl2Enable, &ConsoleCheckbox::checkStateChanged, this, &PkgRaplLimitIVBWidget::onPl2EnableChanged);
    }

    void PkgRaplLimitIVBWidget::setRanges(const PWTS::MinMax &range) const {
        pl1->setRange(range.min, range.max);
        pl2->setRange(range.min, range.max);
    }

    void PkgRaplLimitIVBWidget::setData(const PWTS::DaemonPacket &packet) {
    ...
    void PkgRaplLimitIVBWidget::setDataForPacket(const PWTS::ClientPacket &packet) const {
        if (!isEnabled())
            return;
    ...
    void PkgRaplLimitIVBWidget::onPl1EnableChanged(const Qt::CheckState state) const {
        pl1->setEnabled(state == Qt::Checked);
    }

    void PkgRaplLimitIVBWidget::onPl2EnableChanged(const Qt::CheckState state) const {
