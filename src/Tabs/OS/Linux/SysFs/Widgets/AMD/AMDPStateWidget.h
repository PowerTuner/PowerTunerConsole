...
namespace PWT::CUI {
    class AMDPStateWidget final: public QWidget {
        Q_OBJECT

    private:
        struct WData final {
            bool valid;
            QList<QString> availableEPP;
            QString epp;
        };

        ConsoleSelect *cpuSelect = nullptr;
        ConsoleSelect *pstateStatus = nullptr;
        ConsoleSelect *eppAll = nullptr;
        ConsoleSelect *epp = nullptr;
        QList<WData> data;

    public:
        explicit AMDPStateWidget(int threadCount);
    ...
    private slots:
        ...
        void onEPPAllChanged(const QString &text);
        void onCPUChanged(int idx) const;
        void onEPPChanged(const QString &text);
    };
}
