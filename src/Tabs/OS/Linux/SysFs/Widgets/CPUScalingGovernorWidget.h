...
namespace PWT::CUI {
    class CPUScalingGovernorWidget final: public QWidget {
        Q_OBJECT

    private:
        struct WData final {
            bool valid;
            PWTS::LNX::CPUScalingAvailableGovernors availableGovs;
            QString governor;
        };

        ConsoleSelect *governorAll = nullptr;
        ConsoleSelect *cpuSelect = nullptr;
        ConsoleSelect *governor = nullptr;
        QList<WData> data;
    ...
    private slots:
        void onGovernorAllChanged(const QString &text);
        void onCPUSelectChanged(int idx) const;
        void onGovernorChanged(const QString &text);
    };
}
